...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { date, amount, from } = await req.json();
  ...
  owner.received = (owner.received || 0) + Number(amount);
  owner.history.push({ date, day: getDayName(date), amount: Number(amount), from: from || "N/A", type: "receipt" });
  await owner.save();
